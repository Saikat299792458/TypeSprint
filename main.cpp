...
/**race doesn't end in single mode*/
#include<windows.h>
#include"resource.h"
#include<bits/stdc++.h>
using namespace std;
HFONT f1,f2,f3,f4;
HWND hm=NULL;
int b=0,v,i=0,k,hv=200,sz[5],pd=200,n,s,wc=0,tc=9,inc=-1,ds=-1,hd,hq,we,wt,wa,wg,t2,line,nb,nc,pm,smx=0,poco=0;
int con=0,tuc1,tuc2,ac=1,ind=0,down,bil,knil,ymin=0,ymax=0,ycur=0,tmp,nut,xmin=0,xmax=0,xcur=0,dj=0,nfsz=0;
...
Your score of a successful race is automatically recorded. Scrolling Style and font settings will be \
...
char fonts[5][30]={"Times New Roman","Perpetua","Chiller","Calibri","IMPACT"};
char scores[6][1000];
int spacesize,nfsp=0;
POINT cp,rp,wp;
SCROLLINFO si,si2;
TRACKMOUSEEVENT tme;
MINMAXINFO mima;
...
HWND hh=NULL;
float av,dv;
char a[5][1000],wn[5]="0",tn[6]="00:09",buf[5];
bool wr=false,cbcon=false,tcont=true,tstart=true,wcb=false,did_race_end=false,meridian=false,mousein=false,pdone=false;
bool rcd=false,xdone=false,isshown=false,sp=false;
string mode,f;
    ...
    case WM_PAINT:
        {
            ...
            down+=pm;down+=nc*2;
            SelectObject(hg,f3);
            TextOut(hg,0,down,"About:",6);
            down+=nb;
            SelectObject(hg,f4);
            TextOut(hg,0,down,"App version:1.4.0",17);
            down+=nc;
    ...
    else
    {
        ...
        if(f=="STC")
        {
            ...
            BITMAP bmp;
                ...
                hbw=CreateBitmap(bmp.bmWidth,bmp.bmHeight,1,1,NULL);
                SetBkColor(hd1,RGB(0,0,0));
                SelectObject(hdbw,hbw);
                BitBlt(hdbw,0,0,bmp.bmWidth,bmp.bmHeight,hd1,0,0,SRCCOPY);
                BitBlt(hbt,0,(r1.bottom-bmp.bmHeight)/2,bmp.bmWidth,bmp.bmHeight,hdbw,0,0,SRCAND);
                BitBlt(hbt,0,(r1.bottom-bmp.bmHeight)/2,bmp.bmWidth,bmp.bmHeight,hd1,0,0,SRCPAINT);
                SetRect(&r1,bmp.bmWidth,0,r1.right,r1.bottom);
                SelectObject(hbt,fs);
                SetBkMode(hbt,TRANSPARENT);
...
void kick()
{
    ...
    if(rcd==true)
        CreateWindowEx(WS_EX_CLIENTEDGE,
                           ...
                           0,0,0,0,hm,(HMENU)ID_SINGLE,GetModuleHandle(NULL),NULL);
    SendMessage(hm,WM_SIZE,0,0);
...
void draw(COLORREF cr1,COLORREF crp,char x,char y,char z)
{
    HDC h1;
    if(rcd==true)
    h1=GetDC(GetDlgItem(hm,ID_SINGLE));
    else
    h1=GetDC(hm);
    SelectObject(h1,f1);
    int m;
    ...
    if(rcd==true)
    ReleaseDC(GetDlgItem(hm,ID_SINGLE),h1);
    else
    ReleaseDC(hm,h1);
}
void dre(HWND h)
...
void destroyer()
{
    ...
    DestroyWindow(GetDlgItem(hm,ID_SINGLE));
    //SetTextColor(h1,RGB(0,0,0));
    if(mode=="kick")
        kick();
    else if(mode=="Set")
...
LRESULT CALLBACK Backpro(HWND h,UINT u,WPARAM w,LPARAM l)
{
    ...
    case WM_LBUTTONUP:
        ...
        if(sp==true)
        {
            MessageBox(NULL,"The Interface sucks,right? What can I say, it's made with CodeBlocks IDE using C++ only. Plus I lack imagination.","A Fact",MB_OK|MB_ICONINFORMATION);
            sp=false;
        }
        break;
    default:
...
LRESULT CALLBACK single(HWND h,UINT u,WPARAM w,LPARAM l)
{
    switch(u)
    {
    case WM_PAINT:
        ...
        PAINTSTRUCT psin;
        RECT r3,rb;
        GetClientRect(GetDlgItem(hm,ID_SINGLE),&r3);
        rb=r3;
        HDC h2=BeginPaint(h,&psin),h1=CreateCompatibleDC(0);
        FillRect(h2,&r3,CreateSolidBrush(crf1));
        SelectObject(h2,f1);
        cout<<smx<<endl;
        HBITMAP hbm1=CreateCompatibleBitmap(h2,smx,hq);
        SelectObject(h1,hbm1);
        SetRect(&rb,0,0,smx,hq);
        FillRect(h1,&rb,CreateSolidBrush(crf1));
        int x,y,m,j=0,hj,hk;
            char red[1000],black[1000];
            COLORREF rf1;
            hv=0;
            pd=hv;
            SelectObject(h1,f1);
        ...
        BitBlt(h2,0-xcur,0,smx,hq,h1,0,0,SRCCOPY);
        DeleteDC(h1);
        EndPaint(h,&psin);
        b=0-xcur+poco;
        if(mode=="race"&&cbcon==true)
        {
    ...
    case WM_HSCROLL:
        {
            ...
            int smx2;poco=0;
            HDC h1=GetDC(GetDlgItem(hm,ID_SINGLE));
            SelectObject(h1,f1);
                for(int i=0;a[0][i]!='\0';i++)
                {
                    GetCharWidth32(h1,(UINT)a[0][i],(UINT)a[0][i],&smx2);
                    if(i<cp.x)
                        poco+=smx2;
                }
            ReleaseDC(GetDlgItem(h,ID_SINGLE),h1);
            xNewPos=max(0,xNewPos);
            xNewPos=min(xmax, xNewPos);
            if(xNewPos==xcur)
                break;
            xDelta=xNewPos-xcur;
            xcur=xNewPos;
            ...
            si2.fMask=SIF_POS;
            si2.nPos=xcur;
            SetScrollInfo(h,SB_HORZ,&si2,TRUE);
            break;
        }
    default:
        return DefWindowProc(h,u,w,l);
...
LRESULT CALLBACK windpro(HWND h,UINT u,WPARAM w,LPARAM l)
{
    ...
    int w=0;
    ifstream mp("pos.tsd");
    ifstream md("Data.tsd");
    ifstream ma("Assets.tsd");
    ifstream ms("Scores.tsd");
    if(!mp.is_open()||!md.is_open()||!ma.is_open()||!ms.is_open())
    {
        MessageBox(NULL,"Could not load associative document files(*.tsd). Please check for missing or damaged items or re-unpack the compressed file.","Fatal Error",MB_OK|MB_ICONERROR);
        DestroyWindow(h);
        break;
    }
    while(!ma.eof())
    {
        ...
        stringstream(asb.c_str())>>rcd;
        else if(w==3)
        stringstream(asb.c_str())>>isshown;
        w++;
    }
    mp.close();
    md.close();
    ...
    case WM_SIZE:
        ...
        SendMessage(GetDlgItem(hm,ID_COMBO1),(UINT)CB_ADDSTRING,(WPARAM)0,(LPARAM)fonts[i]);
            ...
            else if(mode=="Info")
            {
                ...
                pm=ri.right*24/100;
                down=0;
                down+=nb*2;down+=(7*hd);down+=hq;down+=pm;down+=nc*6;
                nut=down;
...
/*****************************************************************************************************************/
            ...
            {
                SetWindowPos(GetDlgItem(h,ID_SINGLE),NULL,0,rm.bottom/2,rm.right,hq,SWP_NOZORDER);
            if(rcd==false)
            {
            int x,y,m,j=0,hj,hk;
            ...
            pd=hv;
            SelectObject(h1,f1);
            GetCharWidth32(h1,(UINT)' ',(UINT)' ',&spacesize);
            SetTextColor(h1,RGB(0,255,0));
            SetBkMode(h1,TRANSPARENT);
            if(wp.x==-1)
            {
            ...
            if(rcd==true)
            {
                ...
                nfsz=xcur;
                GetClientRect(GetDlgItem(h,ID_SINGLE),&ri);
                xmax=max(smx-int(ri.bottom),0);
                xcur=min(xcur,xmax);
                si2.cbSize=sizeof(si2);
                ...
                si2.nPage=ri.bottom;
                si2.nPos=xcur;
                SetScrollInfo(GetDlgItem(h,ID_SINGLE),SB_HORZ,&si2,TRUE);
                xdone=false;
    ...
    case WM_CHAR:
        ...
        if(mode=="mnd")
        {
            ...
            {
            f="Start";
            SetButtonLayer(2,GetDlgItem(h,ID_B1));
            SetButtonLayer(0,GetDlgItem(h,ID_B1));
        mousein=false;
        mode="kick";
        destroyer();
        return 0;
        ...
        HideCaret(GetDlgItem(h,ID_SINGLE));
        switch(w)
        {
        case 0x1B:
            did_race_end=true;
        ...
        default:
                        ...
                        if(a[cp.y][cp.x]==' ')
                            {
                                ...
                                {
                                    nfs2=0;
                                    for(int i=dj;i<=cp.x;i++)
                                    {
                                        GetCharWidth32(h1,(UINT)a[0][i],(UINT)a[0][i],&nfs1);
                                        nfs2+=nfs1;
                                    }
                                    dj=cp.x+1;
                                    nfsz+=nfs2;
            ...
            if(wr==true)
            {
                ...
                b=0;
                int var1=xcur;
                    xcur=nfsz;poco=xcur+spacesize;
            ScrollWindowEx(GetDlgItem(h,ID_SINGLE),-(nfsz-var1),0,(CONST RECT*)NULL,
                (CONST RECT *)NULL,(HRGN)NULL,(PRECT)NULL,
                SW_INVALIDATE);
            UpdateWindow(GetDlgItem(h,ID_SINGLE));
            si2.cbSize=sizeof(si2);
            si2.fMask=SIF_POS;
            si2.nPos=xcur;
            SetScrollInfo(GetDlgItem(h,ID_SINGLE),SB_HORZ,&si2,TRUE);
        ...
        ShowCaret(GetDlgItem(h,ID_SINGLE));
        if(did_race_end==true)
            kickdlg(h);
        ReleaseDC(h,h1);
        }
        break;
...
int APIENTRY WinMain(HINSTANCE ar,HINSTANCE b,LPSTR c,int d)
{
    ...
    e.lpszClassName="eb";
    RegisterClassEx(&e);
    e.lpfnWndProc=single;
    e.lpszClassName="single";
    RegisterClassEx(&e);
    e.hbrBackground=HBRUSH(0);
